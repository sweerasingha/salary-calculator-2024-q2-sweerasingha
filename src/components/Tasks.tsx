...
const Tasks: FC<TasksProps> = ({ tasks, className }) => {
  ...
  const TodoCell: FC<{ todo: string; completed: boolean }> = ({ todo, completed }) => (
  ...
  const BaseCustomTooltip = styled(({ className, ...props }: TooltipProps) => (
  ...
  return (
    <div className={`h-[632px] w-full md:w-[656px] bg-white shadow-md flex flex-col border rounded-[8px] ${className}`}>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: '640px', overflow: 'auto' }}>
        <Table sx={{ minWidth: 650, border: 0 }} aria-label="simple table">
          ...
          <TableBody>
            {(rowsPerPage > 0
              ? tasks.slice(page * rowsPerPage - rowsPerPage, page * rowsPerPage)
              : tasks
            ).map((row, index) => (
              <TableRow
                key={row.id || index}
                ...
                >
                  {row.completed ?
                    <CustomTooltip title="Task Completed" arrow>
                      <Image src={completed} alt="Done" width="20" height="20" />
                    </CustomTooltip> :
                    <CustomTooltipYellow title="Task In Progress" arrow>
                      <Image src={inprogress} alt="In Progress" width="20" height="20" />
                    </CustomTooltipYellow>
                  }
                </TableCell>
